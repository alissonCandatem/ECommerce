...
namespace ECommerce.IA.Api.Services
{
  public sealed class QueryCacheService : IQueryCacheService
  {
    ...
    private const float SimilaridadeMinima = 0.98f; // A similaridade vai impactar no SQL gerado, se uma frase for 98% "similar" a uma que ja foi cacheada, vai gerar o SQL(Obs: Isso não é a melhor forma pois podem gerar confusão dependendo do Modelo)
